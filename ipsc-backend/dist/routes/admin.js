...
import { CreateClubSchema, UpdateClubSchema, CreateUserSchema, UpdateUserSchema, fail, ok } from '../types.js';
const router = Router();
// ── Users ─────────────────────────────────────────────────────────────────────
...
router.post('/clubs', (req, res) => {
    ...
    try {
        const existing = db.prepare(`SELECT id FROM clubs WHERE name = ? LIMIT 1`).get(name);
        if (existing) {
...
router.put('/clubs/:id', (req, res) => {
    ...
    try {
        const existing = db.prepare(`SELECT id FROM clubs WHERE id = ?`).get(clubId);
        if (!existing) {
