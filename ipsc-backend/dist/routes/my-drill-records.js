...
function getTemplateByOwner(ownerUserId, templateId) {
    return db
        .prepare(`SELECT * FROM drill_templates WHERE id = ? AND owner_user_id = ?`)
        .get(templateId, ownerUserId);
}
function serializeDrillRecord(row) {
    return {
        id: row.id,
...
function serializeDrillRecordSummary(row) {
    return {
        id: row.id,
        drill_template_id: row.drill_template_id,
        drill_name: row.drill_name ?? row.template_name ?? null,
...
router.post('/drills/:drillId/drill-records', (req, res) => {
    if (!req.user) {
    ...
    try {
        ...
        if (parsed.data.client_drill_result_id) {
           ...
           WHERE owner_user_id = ? AND client_drill_result_id = ?`)
                .get(req.user.id, parsed.data.client_drill_result_id);
            if (existing) {
           ...
           WHERE id = ? AND owner_user_id = ?`).run(template.id, template.name, parsed.data.total_time, parsed.data.num_shots, parsed.data.score ?? null, payloadJson, parsed.data.device_id ?? null, uploadedBy, existing.id, req.user.id);
        ...
        if (recordId === null) {
           ...
           VALUES (NULL, NULL, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                .run(req.user.id, template.id, template.name, parsed.data.total_time, parsed.data.num_shots, parsed.data.score ?? null, payloadJson, parsed.data.client_drill_result_id ?? null, parsed.data.device_id ?? null, uploadedBy);
            recordId = Number(info.lastInsertRowid);
        }
        const row = db
            .prepare(`SELECT r.*, dt.name AS template_name
...
router.get('/drills/:drillId/drill-records', (req, res) => {
    if (!req.user) {
        res.status(401).json(fail('未登录'));
        return;
    }
    ...
    try {
        ...
        const items = db
         ...
         LIMIT ? OFFSET ?`)
            .all(req.user.id, drillId, pageSize, offset);
        res.json(ok({
...
router.get('/drill-records/stats', (req, res) => {
    if (!req.user) {
        res.status(401).json(fail('未登录'));
        return;
    }
    ...
    try {
        ...
        const byDrill = db
         ...
         ORDER BY record_count DESC, drill_name ASC`)
            .all(req.user.id, sinceIso);
        const byDay = db
            .prepare(`SELECT date(created_at) AS date, COUNT(*) AS count, COALESCE(AVG(total_time), 0) AS avg_time
         FROM drill_replays
         WHERE owner_user_id = ? AND created_at >= ?
...
router.get('/drill-records', (req, res) => {
    if (!req.user) {
        res.status(401).json(fail('未登录'));
        return;
    }
...
router.get('/drill-records/:id', (req, res) => {
    if (!req.user) {
        res.status(401).json(fail('未登录'));
        return;
    }
...
router.delete('/drill-records/:id', (req, res) => {
    if (!req.user) {
        res.status(401).json(fail('未登录'));
        return;
    }
