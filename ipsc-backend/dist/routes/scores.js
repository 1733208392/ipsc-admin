...
function getScoreCardPayload(matchId, shooterId, stageId) {
    const shooter = db
...
router.post('/flextarget', (req, res) => {
    ...
    try {
       ...
       ON CONFLICT(shooter_id, stage_id) DO UPDATE SET
         ...
         confirmed = 0,
         updated_at = datetime('now')`).run(matchId, shooter.id, stage.id, total_time, hits.A, hits.C, hits.D, hits.M, cappedNHits, penalties.PE, first_shot ?? null, fastest_split ?? null, totalPoints, hitFactor);
        const score = db
            .prepare(`SELECT * FROM scores WHERE shooter_id = ? AND stage_id = ?`)
...
export function confirmScore(req, res) {
...
export function deleteScore(req, res) {
    const id = Number(req.params['id']);
    try {
        const score = db.prepare(`SELECT * FROM scores WHERE id = ?`).get(id);
        if (!score) {
