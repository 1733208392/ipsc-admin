...
router.post('/flextarget', (req, res) => {
    ...
    try {
       ...
       ON CONFLICT(shooter_id, stage_id) DO UPDATE SET
         ...
         confirmed = 0,
         updated_at = datetime('now')`).run(matchId, shooter.id, stage.id, total_time, hits.A, hits.C, hits.D, hits.M, hits.N, penalties.PE, first_shot ?? null, fastest_split ?? null, totalPoints, hitFactor);
        const score = db
