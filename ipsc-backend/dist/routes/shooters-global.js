...
router.get('/search', (req, res) => {
    ...
    try {
        const like = `%${q}%`;
        const rows = db
            .prepare(`SELECT sg.*, c.name AS default_club_name, c.short_name AS default_club_short_name
...
router.post('/', (req, res) => {
    ...
    try {
        ...
        db.prepare(`INSERT INTO shooters_global
         (uid, name, gender, age, region, default_club_id, id_card, phone, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`).run(uid, name, gender, age ?? null, region ?? null, clubId, id_card ?? null, phone ?? null);
        const shooter = db
            .prepare(`SELECT sg.*, c.name AS default_club_name, c.short_name AS default_club_short_name
         FROM shooters_global sg
         LEFT JOIN clubs c ON c.id = sg.default_club_id
...
router.put('/:uid', (req, res) => {
    ...
    try {
        ...
        values.push(uid);
        db.prepare(`UPDATE shooters_global SET ${fields.join(', ')}, updated_at = datetime('now') WHERE uid = ?`).run(...values);
        const updated = db
            .prepare(`SELECT sg.*, c.name AS default_club_name, c.short_name AS default_club_short_name
         FROM shooters_global sg
         LEFT JOIN clubs c ON c.id = sg.default_club_id
