...
import { CreateSquadSchema, UpdateSquadSchema, AutoAssignSquadsSchema, BatchMoveShootersSchema, AddShooterToSquadSchema, ok, fail, } from '../types.js';
...
router.get('/', (req, res) => {
    ...
    try {
        const squads = db
            .prepare(`SELECT sq.*, COUNT(sh.id) AS shooter_count
         FROM squads sq
...
// DELETE /squads/:id
export function deleteSquad(req, res) {
    const id = Number(req.params['id']);
    try {
        const squad = db.prepare(`SELECT * FROM squads WHERE id = ?`).get(id);
        if (!squad) {
...
export function getSquadQueue(req, res) {
    const matchId = Number(req.params['matchId']);
    try {
        const match = db.prepare(`SELECT id FROM matches WHERE id = ?`).get(matchId);
        if (!match) {
        ...
        const totalStages = db.prepare(`SELECT COUNT(*) AS c FROM stages WHERE match_id = ?`).get(matchId).c;
        const squads = db
            .prepare(`SELECT id, name, sort_order FROM squads WHERE match_id = ? ORDER BY sort_order, id`)
            .all(matchId);
...
export function batchMoveShooters(req, res) {
    const squadId = Number(req.params['squadId']);
    const sourceSquad = db.prepare(`SELECT id, match_id FROM squads WHERE id = ?`).get(squadId);
    if (!sourceSquad) {
        res.status(404).json(fail('Squad not found'));
        return;
    }
    ...
    const targetSquad = db.prepare(`SELECT id, match_id FROM squads WHERE id = ?`).get(parsed.data.target_squad_id);
    if (!targetSquad) {
    ...
    const moveTx = db.transaction((shooterIds) => {
        const updateStmt = db.prepare(`UPDATE shooters SET squad_id = ? WHERE id = ? AND squad_id = ? AND match_id = ?`);
        let moved = 0;
...
export function removeShooterFromSquad(req, res) {
    const squadId = Number(req.params['squadId']);
    const shooterId = Number(req.params['shooterId']);
    const squad = db.prepare(`SELECT id, match_id FROM squads WHERE id = ?`).get(squadId);
    if (!squad) {
        res.status(404).json(fail('Squad not found'));
        return;
    }
...
export function addShooterToSquad(req, res) {
    const squadId = Number(req.params['squadId']);
    const squad = db.prepare(`SELECT id, match_id FROM squads WHERE id = ?`).get(squadId);
    if (!squad) {
        res.status(404).json(fail('Squad not found'));
