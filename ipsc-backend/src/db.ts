...
function getTableColumns(tableName: string): Array<{ name: string; notnull: number }> {
...
function ensureDrillTemplateSchema() {
  ...
  const beforeCount = (db.prepare(`SELECT COUNT(*) AS c FROM drill_templates`).get() as { c: number }).c;
...
function ensureDrillReplaySchema() {
  ...
  const beforeCount = (db.prepare(`SELECT COUNT(*) AS c FROM drill_replays`).get() as { c: number }).c;
  ...
  const afterCount = (db.prepare(`SELECT COUNT(*) AS c FROM drill_replays`).get() as { c: number }).c;
  if (beforeCount !== afterCount) {
...
try {
  ...
  const scoreIndexes = db.prepare(`PRAGMA index_list(scores)`).all() as Array<{
    name: string;
    unique: number;
  }>;
