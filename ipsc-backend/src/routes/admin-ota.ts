...
import {
  fail,
  ok,
  OtaAdminListQuerySchema,
  OtaCreatePackageSchema,
  OtaUpdatePackageSchema,
} from '../types.js';
import {
  archiveOtaPackage,
  computeFileSha1Hex,
...
router.post('/packages', upload.single('file'), async (req: Request, res: Response) => {
  ...
  try {
    const existing = db.prepare(`SELECT id FROM ota_packages WHERE version = ? LIMIT 1`).get(parsed.data.version) as { id: number } | undefined;
    if (existing) {
      if (fs.existsSync(req.file.path)) {
        fs.unlinkSync(req.file.path);
      }
