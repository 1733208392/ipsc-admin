...
router.delete('/clubs/:id', (req: Request, res: Response) => {
  ...
  try {
    ...
    const userCount = (
      db.prepare(`SELECT COUNT(*) AS c FROM users WHERE club_id = ?`).get(id) as { c: number }
    ).c;
    const matchCount = (
      db.prepare(`SELECT COUNT(*) AS c FROM matches WHERE club_id = ?`).get(id) as { c: number }
    ).c;
...
router.get('/users', (_req: Request, res: Response) => {
  try {
    const users = db
      .prepare(
        `SELECT u.id, u.username, u.role, u.club_id, u.name, u.phone, u.status, u.last_login_at,
...
router.post('/users', (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = db
      .prepare(
        `SELECT id, username, role, club_id, name, phone, status, last_login_at, created_at, updated_at
         FROM users WHERE id = ?`
      )
...
router.put('/users/:id', (req: Request, res: Response) => {
  ...
  try {
    const user = db.prepare(`SELECT * FROM users WHERE id = ?`).get(id) as { role: string } | undefined;
    if (!user) {
...
router.get('/matches', (_req: Request, res: Response) => {
  try {
    const matches = db
      .prepare(
        `SELECT m.*, c.name AS club_name, c.short_name AS club_short_name,
