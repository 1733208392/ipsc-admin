...
router.post('/send-code', async (req: Request, res: Response) => {
  ...
  if (purpose === 'register') {
    const existing = db
      ...
      .get(normalizedTarget) as { id: number } | undefined;
    if (existing) {
...
router.post('/register/email', async (req: Request, res: Response) => {
  ...
  const existing = db.prepare(`SELECT id FROM users WHERE email = ? LIMIT 1`).get(normalizedEmail) as { id: number } | undefined;
  if (existing) {
...
router.post('/verify-email', (req: Request, res: Response) => {
  ...
  if (purpose === 'register') {
    ...
    let payload: { email: string; password_hash: string; name: string; locale: string };
    try {
    ...
    try {
      const userId = db.transaction(() => {
        ...
        const userInfo = db
          .prepare(
            `INSERT INTO users (username, password_hash, role, club_id, name, email, email_verified_at, locale, status, created_at, updated_at)
             VALUES (?, ?, 'club_admin', ?, ?, ?, datetime('now'), ?, 'active', datetime('now'), datetime('now'))`
          )
          .run(
            normalizedEmail,
            payload.password_hash,
            Number(clubInfo.lastInsertRowid),
            payload.name,
            normalizedEmail,
            payload.locale
          );
        ...
        // Link identity
        db.prepare(
          `INSERT INTO user_identities (user_id, provider, provider_uid, provider_email, linked_at, last_used_at)
           VALUES (?, 'email', ?, ?, datetime('now'), datetime('now'))`
        ).run(newUserId, normalizedEmail, normalizedEmail);

        return newUserId;
      })();

      const freshUser = db.prepare(`SELECT * FROM users WHERE id = ?`).get(userId) as Express.User;
      res.status(201).json(buildAuthResponse(freshUser));
      return;
    } catch (err) {
  ...
  if (purpose === 'login') {
    ...
    const user = db.prepare(`SELECT * FROM users WHERE email = ? AND status = 'active'`).get(normalizedEmail) as Express.User | undefined;
    if (!user) {
...
router.post('/login/email', (req: Request, res: Response) => {
  ...
  const user = db
    ...
    .get(normalizedEmail, normalizedEmail) as (Express.User & { password_hash: string }) | undefined;
...
router.post('/reset-password', (req: Request, res: Response) => {
  ...
  const user = db.prepare(`SELECT * FROM users WHERE email = ? LIMIT 1`).get(normalizedEmail) as Express.User | undefined;
  if (!user) {
    res.status(404).json(fail('账号不存在'));
    return;
  }
...
router.post('/register/phone', async (req: Request, res: Response) => {
  ...
  const existing = db.prepare(`SELECT id FROM users WHERE phone = ? LIMIT 1`).get(phone) as { id: number } | undefined;
  if (existing) {
...
router.post('/verify-phone', (req: Request, res: Response) => {
  ...
  if (purpose === 'register') {
    ...
    let payload: { phone: string; password_hash: string; name: string; locale: string };
    try {
      payload = JSON.parse(Buffer.from(token, 'base64url').toString('utf8'));
    } catch {
      res.status(400).json(fail('注册上下文无效，请重新注册'));
    ...
    try {
      const userId = db.transaction(() => {
        ...
        const userInfo = db
          .prepare(
            `INSERT INTO users (username, password_hash, role, club_id, name, phone, phone_verified_at, locale, status, created_at, updated_at)
             VALUES (?, ?, 'club_admin', ?, ?, ?, ?, datetime('now'), ?, 'active', datetime('now'), datetime('now'))`
          )
          .run(
            phone,
        ...
        db.prepare(
          `INSERT INTO user_identities (user_id, provider, provider_uid, linked_at, last_used_at)
           VALUES (?, 'phone', ?, datetime('now'), datetime('now'))`
        ).run(newUserId, phone);
  ...
  if (purpose === 'login') {
    const user = db.prepare(`SELECT * FROM users WHERE phone = ? AND status = 'active'`).get(phone) as Express.User | undefined;
    if (!user) {
      res.status(404).json(fail('账号不存在'));
      return;
    }
...
router.post('/login/phone', (req: Request, res: Response) => {
  ...
  const user = db
    ...
    .get(phone, phone) as (Express.User & { password_hash: string }) | undefined;
...
router.post('/reset-password/phone', (req: Request, res: Response) => {
  ...
  const user = db.prepare(`SELECT * FROM users WHERE phone = ? LIMIT 1`).get(phone) as Express.User | undefined;
  if (!user) {
    res.status(404).json(fail('账号不存在'));
    return;
  }
