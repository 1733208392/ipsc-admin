...
export function updateDivision(req: Request, res: Response): void {
...
export function deleteDivision(req: Request, res: Response): void {
  const id = Number(req.params['id']);
  try {
    const division = db.prepare(`SELECT * FROM divisions WHERE id = ?`).get(id);
    if (!division) {
    ...
    const shooterCount = (
      db.prepare(`SELECT COUNT(*) as c FROM shooters WHERE division_id = ?`).get(id) as { c: number }
    ).c;
    if (shooterCount > 0) {
