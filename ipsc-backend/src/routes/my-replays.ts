...
router.post('/drills/:drillId/replays', (req: Request, res: Response) => {
  if (!req.user) {
...
router.get('/drills/:drillId/replays', (req: Request, res: Response) => {
  if (!req.user) {
    res.status(401).json(fail('未登录'));
    return;
  }
...
router.get('/replays/stats', (req: Request, res: Response) => {
  if (!req.user) {
    res.status(401).json(fail('未登录'));
    return;
  }
...
router.delete('/replays/:id', (req: Request, res: Response) => {
  if (!req.user) {
    res.status(401).json(fail('未登录'));
    return;
  }
