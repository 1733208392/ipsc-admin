...
export interface RankedStageShooter {
  shooter_id: number;
  division_id: number;
  stage_id: number;
  hit_factor: number;
...
function getShooterBaseList(matchId: number, divisionId: number | null, categoryFilter?: CategoryFilter): ShooterBase[] {
  let sql = `
    SELECT
      s.id,
      s.name,
  ...
  return db.prepare(sql).all(...params) as ShooterBase[];
}

function getStagePoints(stageId: number): number {
  const stage = db
...
function getDivisionStageScores(matchId: number, stageId: number, shooterIds: number[]): StageScoreRow[] {
...
export function calculateStageRanking(
  ...
  const divisionGroups = new Map<number, number[]>();
  ...
  const ranked: RankedStageShooter[] = [];
...
export function calculateOverallRanking(
  ...
  const stages = db
    ...
    .all(matchId) as Array<{ id: number; stage_points: number }>;

  const shooters = getShooterBaseList(matchId, divisionId, categoryFilter);
  if (shooters.length === 0) return [];

  const divisionGroups = new Map<number, ShooterBase[]>();
  for (const shooter of shooters) {
    if (!divisionGroups.has(shooter.division_id)) {
      divisionGroups.set(shooter.division_id, []);
    }
  ...
  const shooterStagePoints = new Map<number, Map<number, StageDetail>>();
