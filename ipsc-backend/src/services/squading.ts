...
export type SortBy = 'registration' | 'bib' | 'division' | 'random' | 'region' | 'club';
export type Strategy = 'sequential' | 'snake' | 'division_balanced';
...
export function autoAssignSquads(options: AutoAssignOptions) {
  ...
  let shooters: ShooterRow[];

  if (clear_existing) {
    const hasScores = db.prepare(`SELECT COUNT(*) AS c FROM scores WHERE match_id = ?`).get(matchId) as { c: number };
    if (hasScores.c > 0) {
    ...
    shooters = db
      .prepare(
        `SELECT s.id, s.created_at, s.bib_number, s.region, s.club, d.code AS division_code
         FROM shooters s
         JOIN divisions d ON s.division_id = d.id
         WHERE s.match_id = ?`
      )
      .all(matchId) as ShooterRow[];
  } else {
    shooters = db
      .prepare(
        `SELECT s.id, s.created_at, s.bib_number, s.region, s.club, d.code AS division_code
  ...
  if (strategy === 'sequential') {
    ...
    const buckets = new Map<string, ShooterRow[]>();
  ...
  const maxSort = db.prepare(`SELECT COALESCE(MAX(sort_order), -1) AS m FROM squads WHERE match_id = ?`).get(matchId) as { m: number };
  ...
  const unassigned = (
    db.prepare(`SELECT COUNT(*) AS c FROM shooters WHERE match_id = ? AND squad_id IS NULL`).get(matchId) as { c: number }
  ).c;
...
function sortShooters(shooters: ShooterRow[], sort_by: SortBy): ShooterRow[] {
  const copy = [...shooters];
